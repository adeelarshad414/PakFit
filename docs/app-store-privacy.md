...
- `NSPrivacyAccessedAPITypes`: declares `NSPrivacyAccessedAPICategoryUserDefaults` with reason `CA92.1` because the app reads and writes app-only local snapshot data through `UserDefaults`.
