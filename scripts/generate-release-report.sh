...
IOS_SETUP_PARITY_DOC_STATUS="missing"
IOS_SETUP_PARITY_DOC_SHA256=""
if [[ -f "$IOS_SETUP_PARITY_DOC_FILE" ]]; then
  IOS_SETUP_PARITY_DOC_STATUS="$IOS_SETUP_PARITY_DOC_FILE"
  IOS_SETUP_PARITY_DOC_SHA256="$(sha256_file "$IOS_SETUP_PARITY_DOC_FILE")"
fi
IOS_HEALTH_MARKER_PARITY_STATUS="not checked"
if bash scripts/validate-ios-health-marker-parity.sh >/dev/null 2>&1; then
  IOS_HEALTH_MARKER_PARITY_STATUS="passed"
fi
IOS_HEALTH_MARKER_PARITY_DOC_STATUS="missing"
IOS_HEALTH_MARKER_PARITY_DOC_SHA256=""
if [[ -f "$IOS_HEALTH_MARKER_PARITY_DOC_FILE" ]]; then
...
PCOS_CLINICAL_SAFETY_STATUS="not checked"
if bash scripts/validate-pcos-clinical-safety.sh >/dev/null 2>&1; then
  PCOS_CLINICAL_SAFETY_STATUS="passed"
fi
PCOS_CLINICAL_SAFETY_DOC_STATUS="missing"
PCOS_CLINICAL_SAFETY_DOC_SHA256=""
if [[ -f "$PCOS_CLINICAL_SAFETY_DOC_FILE" ]]; then
  PCOS_CLINICAL_SAFETY_DOC_STATUS="$PCOS_CLINICAL_SAFETY_DOC_FILE"
  PCOS_CLINICAL_SAFETY_DOC_SHA256="$(sha256_file "$PCOS_CLINICAL_SAFETY_DOC_FILE")"
fi
PREGNANCY_PLAN_SAFETY_STATUS="not checked"
if bash scripts/validate-pregnancy-plan-safety.sh >/dev/null 2>&1; then
  PREGNANCY_PLAN_SAFETY_STATUS="passed"
fi
PREGNANCY_PLAN_SAFETY_DOC_STATUS="missing"
PREGNANCY_PLAN_SAFETY_DOC_SHA256=""
if [[ -f "$PREGNANCY_PLAN_SAFETY_DOC_FILE" ]]; then
...
KIDNEY_PLAN_SAFETY_STATUS="not checked"
if bash scripts/validate-kidney-plan-safety.sh >/dev/null 2>&1; then
  KIDNEY_PLAN_SAFETY_STATUS="passed"
fi
KIDNEY_PLAN_SAFETY_DOC_STATUS="missing"
KIDNEY_PLAN_SAFETY_DOC_SHA256=""
if [[ -f "$KIDNEY_PLAN_SAFETY_DOC_FILE" ]]; then
...
IOS_ANALYSIS_DASHBOARD_PARITY_DOC_STATUS="missing"
IOS_ANALYSIS_DASHBOARD_PARITY_DOC_SHA256=""
if [[ -f "$IOS_ANALYSIS_DASHBOARD_PARITY_DOC_FILE" ]]; then
...

{
  ...
  echo "- iOS setup parity doc SHA-256: ${IOS_SETUP_PARITY_DOC_SHA256:-missing}"
  echo "- iOS setup parity gate: $IOS_SETUP_PARITY_STATUS"
  echo "- iOS setup parity policy: Setup tab, theme/profile/goal/routine/diet/training controls, lifestyle toggles, medical caution toggles, adult age range, profile writeback, docs, and store listing copy checked"
  echo "- iOS setup parity boundary: iPhone device QA, VoiceOver, large-text review, and signed production build review remain external before public release"
  echo
  echo "## iOS Health Marker Parity"
  echo
  echo "- iOS health marker parity doc: $IOS_HEALTH_MARKER_PARITY_DOC_STATUS"
  echo "- iOS health marker parity doc SHA-256: ${IOS_HEALTH_MARKER_PARITY_DOC_SHA256:-missing}"
  echo "- iOS health marker parity gate: $IOS_HEALTH_MARKER_PARITY_STATUS"
  ...
  echo
  echo "## PCOS Clinical Safety"
  echo
  echo "- PCOS clinical safety doc: $PCOS_CLINICAL_SAFETY_DOC_STATUS"
  echo "- PCOS clinical safety doc SHA-256: ${PCOS_CLINICAL_SAFETY_DOC_SHA256:-missing}"
  echo "- PCOS clinical safety gate: $PCOS_CLINICAL_SAFETY_STATUS"
  echo "- PCOS clinical safety policy: Android/iOS PCOS risk factors, metabolic/reproductive insight, clinician-review actions, official source categories, no diagnosis wording, and no self-started medicine/supplement guidance checked"
  echo "- PCOS clinical safety boundary: clinical copy review, device QA, fertility/medication review, legal/privacy review, and store age-suitability review remain external before public release"
  echo
  echo "## Pregnancy Plan Safety"
  echo
  echo "- Pregnancy plan safety doc: $PREGNANCY_PLAN_SAFETY_DOC_STATUS"
  echo "- Pregnancy plan safety doc SHA-256: ${PREGNANCY_PLAN_SAFETY_DOC_SHA256:-missing}"
  echo "- Pregnancy plan safety gate: $PREGNANCY_PLAN_SAFETY_STATUS"
  ...
  echo
  echo "## Kidney Plan Safety"
  echo
  echo "- Kidney plan safety doc: $KIDNEY_PLAN_SAFETY_DOC_STATUS"
  echo "- Kidney plan safety doc SHA-256: ${KIDNEY_PLAN_SAFETY_DOC_SHA256:-missing}"
  echo "- Kidney plan safety gate: $KIDNEY_PLAN_SAFETY_STATUS"
  ...
  echo "- iOS analysis dashboard parity doc SHA-256: ${IOS_ANALYSIS_DASHBOARD_PARITY_DOC_SHA256:-missing}"
  echo "- iOS analysis dashboard parity gate: $IOS_ANALYSIS_DASHBOARD_PARITY_STATUS"
